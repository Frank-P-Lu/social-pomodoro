defmodule SocialPomodoroWeb.LobbyLive do
  ...
  def mount(_params, _session, socket) do
    ...
    username = generate_friendly_username()
  ...
  def handle_event("create_room", _params, socket) do
    {:ok, room_id} = SocialPomodoro.RoomRegistry.create_room(
      socket.assigns.username,
      socket.assigns.duration_minutes
    )

    {:noreply, push_navigate(socket, to: ~p"/room/#{room_id}?username=#{socket.assigns.username}")}
  end

  @impl true
  def handle_event("join_room", %{"room-id" => room_id}, socket) do
    case SocialPomodoro.Room.join(room_id, socket.assigns.username) do
      :ok ->
        {:noreply, push_navigate(socket, to: ~p"/room/#{room_id}?username=#{socket.assigns.username}")}
      {:error, _reason} ->
          ...
          <div class="space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-8">
              ...
              <div class="text-gray-600 space-y-3">
                ...
                <p>Join a room or create your own, set a timer, and work alongside others in real-time.</p>
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex-1">
                        ...
                        <div class="text-sm text-gray-600">
                          <%= length(room.participants) %> <%= if length(room.participants) == 1, do: "person", else: "people" %>
